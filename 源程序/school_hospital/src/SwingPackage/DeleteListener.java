...
//	}
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		String[] sType={"","医生","病人","病房","科室","药房","药品","住院信息","处方信息","就诊信息"};
		jName[0]=new JLabel("类型选择");
		jName[1]=new JLabel("编号");
...
//		jifShow.add(displayDown,BorderLayout.SOUTH);
		...
		JOptionPane.showMessageDialog(jplDisplay, "请选择相应类型进行查看","提示", 1);
		//为jbDelete按钮添加监听器
		jbDelete.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				{
					JOptionPane.showMessageDialog(jifShow, "请选择相应类型","提示",1);
					return;
				}
				...
				{
					JOptionPane.showMessageDialog(jifShow, "请输入相应类型的编号","提示",1);
					return;
				}
				try{
...
//						resTable.setEnabled(true);		//设置表格能被编辑
						...
						Object[] value={"是","否","取消"};
						int result=JOptionPane.showOptionDialog(mySrollPane, "确定删除此信息", "警告", 2, 2,null, value, value[0]);
						if(result==0)
						{
							...
							mySrollPane.add(resTable);
							mySrollPane.setViewportView(resTable);
							JOptionPane.showOptionDialog(mySrollPane, "您的删除已成功！", "提示", 2, 2,null, value, value[0]);
						}						
					}
					else
					{
						...
						mySrollPane.add(resTable);
						mySrollPane.setViewportView(resTable);
						JOptionPane.showMessageDialog(jifShow, "此信息不存在","错误",1);
					}
